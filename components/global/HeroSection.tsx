...
const HeroSection: React.FC<HeroSectionProps> = ({ title, description, imageUrl, wide, maxWidth, backgroundImage, darkThemed, gradient, button }) => {
    ...
    return (
        <section
            ...
            <main className={`flex w-full flex-col px-5 md:px-0 ${imageUrl && "gap-y-12"} ${backgroundImage} ${darkThemed ? "text-white" : gradient ? "bg-transparent" : "bg-yellow-50"}`}>
                <div className={`w-full flex flex-col justify-center items-center gap-y-12 ${imageUrl ? "py-14" : "h-[650px]"}`}>
                    <div className={`${wide ? "w-[1000px]" : maxWidth ? "max-w-[1440px]" : "w-[720px]"} mx-auto flex flex-col py-5 justify-between items-center gap-6 text-center ${isVisible ? 'animate-fade-in-up' : 'opacity-0'}`}>
                        ...
                        <p className="text-[17px] md:text-2xl w-full leading-tight md:leading-normal transition-colors duration-300">{description}</p>
                    </div>
