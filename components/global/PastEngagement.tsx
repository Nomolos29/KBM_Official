            ...
            {team.map((image, index) => (
              <div
                ...
                className={`flex-shrink-0 w-auto p-5 transition-transform duration-700 ${
                  activeIndex === index
                    ? "bg-gradient-to-r from-[#ffe8b68c] to-[#E2E2E24D] absolute scale-[1.15] top-24 right-[calc(100%/3-12px)] md:right-[34%] xl:right-[calc(24%)] z-10 border-white border shadow-md"
                    : "opacity-15"
                }`}
                onClick={() => setActiveIndex(index)}
              >
                <Image src={image} alt='teams image' width={0} height={0} className="w-full h-auto" />
              </div>
            ))}
          </div>
